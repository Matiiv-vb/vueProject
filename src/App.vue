<template>
      ...
      <button v-on:click="clickHeight = !clickHeight">Heigth</button>
			<form @submit.prevent="checkDis()" >
				<div class="form-group">
					<label>Email</label>
					<input type="text" class="form-control" v-model.trim="email" ref="fInput">
          <label>Phone</label>
          ...
          <label>Last Name</label>
				...
				<div class="form-group" >
					<label>Guests</label>
					<input type="button" class="btn btn-primary" value="+" v-on:click="add"> 
				</div>
				<div class="form-group" v-for="(i, ind) in guest" v-bind:key="ind" >
          <label>Guest {{ ind }}
          </label>
          <input type="text"   v-model="guest[ind].v" ref="lastInp" >
          <button class="btn btn-danger" v-on:click="$delete(guest, ind)" >del</button> 
				</div>
				<hr>
				<button class="btn btn-success my-button" v-on:click="checkGuest()" v-bind:disabled="!checkDis()">Send Data</button>
			</form>
			<div>
				<h2>All done!</h2>
				<table class="table table-bordered" >
					...
					<tr>
						<td v-for="(g, index) in guest" v-bind:key="index" >Guests {{index}} {{guest[index].v}} </td>
					</tr>
				</table>
        ...
        <div class="form-check">
          <div>
            <label class="form-check-label">
              <input class="form-check-input" type="radio" value="blu" v-model="cl"> 
              Blue
            </label>
          </div>
          <div>
            <label class="form-check-label">
              <input class="form-check-input" type="radio" value="orang" v-model="cl"> 
              Orange
              </label>
              </div>
              <div>
                <label class="form-check-label">
                  <input class="form-check-input" type="radio" value="red" v-model="cl"> 
                  Red
                </label>
              </div>
          </div>
  ...
  <div class="wrapper">
        ...
        <div v-show="scrolledToBottom">
				...
				<div class="form-check">
					<label class="form-check-label" for="checkbox">
						<input class="form-check-input" type="checkbox" v-model="checked2" > 
						2ch {{ checked }}
					</label>
				</div>
			<div v-show="checked2">
        <div>
          <div class="form-check">
            <label class="form-check-label" for="one">
              <input class="form-check-input" id="one" type="radio" name="rad"> 
              Mail
            </label>
          </div>
        </div>
        <div>
          <label class="form-check-label" for="two">
            <input class="form-check-input" id="two" type="radio" name="rad"> 
            Phone
          </label>
        </div>
      </div>
  ...
  methods: {
    ...
    scroll (e) {
      ...
      div.onscroll = () => {
        ...
        let bottomOfWindow = div.scrollTop  + div.getBoundingClientRect().height > div.scrollHeight
